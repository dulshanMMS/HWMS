...
export const validateUserName = (userName) => {
  ...
  if (trimmed.length > 100) {
    return { valid: false, error: "User name cannot exceed 100 characters" };
  }
...
export const validateBookingData = (data) => {
  const { userName, teamColor, color, date, entryTime, exitTime, floor, teamName, roomId, bookedBy, status } = data;
  ...
  if (!userNameValidation.valid) errors.push(userNameValidation.error);
...
export const validateSeatData = (seatData) => {
  ...
  const userNameValidation = validateUserName(seatData.userName);
  if (!userNameValidation.valid) errors.push(userNameValidation.error);
